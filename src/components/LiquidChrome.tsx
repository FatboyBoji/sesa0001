...
function LiquidChromeFallback({ baseColor = [0.1, 0.1, 0.1] }: { baseColor?: [number, number, number] }) {
  const gradientStyle = {
    background: `linear-gradient(45deg, 
      rgb(${baseColor[0] * 255}, ${baseColor[1] * 255}, ${baseColor[2] * 255}),
      rgb(${baseColor[0] * 255 + 50}, ${baseColor[1] * 255 + 50}, ${baseColor[2] * 255 + 50}))`
  };
  ...
  return (
    <div 
      ...
      style={gradientStyle}
    >
      <p className="text-white text-lg md:text-xl font-medium mb-2 animate-fade-in">
        3D Effects Not Available
      </p>
      <p className="text-white/80 text-sm md:text-base animate-fade-in-delay">
        Please update your browser to see the full experience
      </p>
    </div>
  );
}
...
export const LiquidChrome: React.FC<LiquidChromeProps> = ({
  ...
  }, [baseColor, speed, amplitude, frequencyX, frequencyY, interactive, hasWebGL]);

  // Show fallback if WebGL is not supported
  if (hasWebGL === false) {
    return <LiquidChromeFallback baseColor={baseColor} />;
  }
